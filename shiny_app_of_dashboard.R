...
library(forecast)
# UI
ui <- navbarPage("Global Temperature Analysis",
                 
                 # First Tab: Global Average Temperature Rise
                 tabPanel("Global Average Temperature Rise",
                          fluidPage(
                            ...
                            sidebarLayout(
                              sidebarPanel(
                                ...
                                actionButton("info_button_tab1", icon("info-circle"))  # Info button for Tab 1
                              ),
                              mainPanel(
                 ...
                 # Second Tab: Indicators of Global Surface Warming
                 tabPanel("Global Surface Warming from 1950-2022", 
                          fluidPage(
                            titlePanel("Global Surface Warming Datasets"),
                            sidebarLayout(
                              sidebarPanel(
                                selectInput("indicator", "Choose an dataset", 
                                ...
                                radioButtons("showAll", "Display Options", 
                                             choices = c("Show Selected dataset" = "single", 
                                                         "Show All datasets" = "all"),
                                             selected = "single"),
                                actionButton("info_button_tab2", icon("info-circle"))  # Info button for Tab 2
                 ...
                 # Third Tab: Current Global Surface Warming
                 tabPanel("Current Global Warming Estimates", 
                          fluidPage(
                            titlePanel("Current Global Surface Warming"),
                            mainPanel(
                              plotOutput("warmingPlot", height = "50vh"),
                              actionButton("info_button_tab3", icon("info-circle"))  # Info button for Tab 3
                            )
                          )
                 ),
                 
                 # Fourth Tab: Climate Information (RMarkdown Content)
                 tabPanel("Climate Information",
                          fluidPage(
                            titlePanel("Climate Information"),
                            mainPanel(
                              uiOutput("rmdContent")
                            )
                          )
...
server <- function(input, output, session) {
  ...
  # UI for selecting the start and end years of the pre-industrial period
  ...
  # Reactive function to calculate values based on the selected dataset and method
  calcValues <- reactive({
    req(input$dataName, input$method)
    data_name <- input$dataName
    method <- input$method
    
    # Load the dataset using data_name
  ...
  allIndicators <- reactive({
    ...
    indicator_data <- lapply(files, function(file) {
      read_csv(paste0("data/indicator_data/", file), show_col_types = FALSE) %>%
        mutate(Indicator = gsub("\\.csv", "", file))  # Remove ".csv" from Indicator names
    })
  ...
  output$indicatorPlot <- renderPlot({
    ...
    if (input$showAll == "all") {
      ...
      ggplot() +
        ...
        scale_x_continuous(breaks = seq(min(obs_data$Year), max(obs_data$Year), by = 10)) +  # Add this line for 10-year intervals
        theme(legend.position = "bottom") +
        guides(color = guide_legend(title = NULL)) +
        ggtitle("Global Surface Warming: Observations and All Datasets")
      
    } else {
      indicator_data <- indicatorData()
      indicator_name <- gsub("\\.csv", "", input$indicator)  # Extract the indicator name
  ...
  observeEvent(input$info_button_tab1, {
    info_text_tab1 <- readLines("text/tab1.txt")  # Path to Tab 1 info text file
  ...
  observeEvent(input$info_button_tab2, {
    info_text_tab2 <- readLines("text/tab2.txt")  # Path to Tab 2 info text file
    showModal(modalDialog(
      title = "Global Surface Warming Datasets",
  ...
  observeEvent(input$info_button_tab3, {
    info_text_tab3 <- readLines("text/tab3.txt")  # Path to Tab 3 info text file
    showModal(modalDialog(
