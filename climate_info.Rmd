...
|:----------------------:|---------------|
| **LOESS**              | Locally Estimated Scatterplot Smoothing (LOESS) is a non-parametric regression method that creates a smooth line through a scatterplot by fitting simple models to localized subsets of the data. This method is useful for capturing trends in the data without assuming a specific functional form. |
| **Spline**             | A spline is a flexible, piecewise polynomial function that is used to fit smooth curves to data. In this context, a penalized cubic regression spline is applied to the data, which provides a smooth fit while controlling for overfitting by penalizing the roughness of the spline. |
| **AR1**                | An Auto-Regressive model of order 1 (AR1) assumes that the current value in a timeseries is linearly dependent on the immediately preceding value, with an added noise component. It is used for modeling serial correlation in time-series data. |
| **ARIMA**              | Auto-Regressive Integrated Moving Average (ARIMA) is a popular statistical model used to forecast and analyze time-series data. It combines three components: autoregression (AR), differencing (I) to make the data stationary, and a moving average (MA) to model the noise. |
| **COR**                | Cubic Orthogonal Regression (COR) involves fitting a cubic polynomial regression model to the data, where the polynomial terms are orthogonalized. This method is used to model non-linear relationships in the data. |
| **OSMA10**             | One-Sided Moving Average over 10 years (OSMA10) smooths the data by averaging the values in a 10-year window, only using past data points. This method is useful for emphasizing longer-term trends by reducing short-term fluctuations. |
| **OSMA20**             | One-Sided Moving Average over 20 years (OSMA20) is similar to OSMA10 but uses a 20-year window, providing an even smoother trend line that emphasizes long-term changes. |
| **30yrlt**             | The 30-Year Linear Trend (30yrlt) method calculates the slope of a linear regression over the most recent 30 years of data. This provides an estimate of the rate of change over a longer period, often used to understand persistent trends. |
| **20yrlt**             | The 20-Year Linear Trend (20yrlt) is similar to the 30-Year Linear Trend but uses a shorter, 20-year period. This method is useful for capturing more recent trends while still considering a substantial time window. |
